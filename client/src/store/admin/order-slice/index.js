...
export const updateOrderStatus = createAsyncThunk(
  ...
  async ({ id, orderStatus }, { rejectWithValue }) => {
    try {
      const response = await axios.put(`${API_BASE}/update/${id}`, { orderStatus });
...
export const updatePaymentStatus = createAsyncThunk(
  ...
  async ({ id, paymentStatus }, { rejectWithValue }) => {
    try {
      const response = await axios.put(`${API_BASE}/update-payment/${id}`, { paymentStatus });
...
export const createSteadfastParcelForOrder = createAsyncThunk(
  ...
  async ({ id, recipient_name, delivery_type = 0 }, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(err?.response?.data || { message: "Failed to create Steadfast parcel" });
    }
  }
);
...
export const syncSteadfastStatusForOrder = createAsyncThunk(
  ...
  async ({ id }, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(err?.response?.data || { message: "Failed to sync Steadfast status" });
    }
  }
);
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updateOrderStatus.fulfilled, (state, action) => {
        ...
        // update list for instant UI
        state.orderList = (state.orderList || []).map((o) =>
          o._id === id ? { ...o, orderStatus, orderUpdateDate: new Date().toISOString() } : o
        );
      })
      ...
      .addCase(updatePaymentStatus.fulfilled, (state, action) => {
        state.actionLoading = false;
        state.successMessage = action?.payload?.message || "Payment status updated";
        ...
        state.orderList = (state.orderList || []).map((o) =>
          o._id === id ? { ...o, paymentStatus, orderUpdateDate: new Date().toISOString() } : o
        );
      })
      ...
      .addCase(createSteadfastParcelForOrder.fulfilled, (state, action) => {
        state.actionLoading = false;
        state.successMessage = action?.payload?.message || "Steadfast parcel created";

        const updatedOrder = action?.payload?.data; // your backend returns data: order
        if (updatedOrder?._id) {
          // Update orderDetails immediately
      ...
      .addCase(createSteadfastParcelForOrder.rejected, (state, action) => {
        state.actionLoading = false;
        state.error = action?.payload?.message || "Failed to create Steadfast parcel";
      })
      ...
      .addCase(syncSteadfastStatusForOrder.fulfilled, (state, action) => {
        state.actionLoading = false;
        state.successMessage = action?.payload?.message || "Steadfast status synced";

        // Your sync endpoint returns {data:{...}}; could be order or a small object.
        // We'll safely update orderDetails/list if we can.
        const payloadData = action?.payload?.data;

        // If backend returns full order in data
        if (payloadData?._id) {
        ...
        // If backend returns partial { orderId, deliveryStatus, orderStatus, steadfast }
        const orderId = payloadData?.orderId;
        if (orderId) {
          if (state.orderDetails && state.orderDetails._id === orderId) {
            state.orderDetails.orderStatus = payloadData.orderStatus || state.orderDetails.orderStatus;
            state.orderDetails.steadfast = payloadData.steadfast || state.orderDetails.steadfast;
          }
          state.orderList = (state.orderList || []).map((o) =>
            o._id === orderId
              ? { ...o, orderStatus: payloadData.orderStatus || o.orderStatus, steadfast: payloadData.steadfast || o.steadfast }
              : o
          );
        }
      })
      .addCase(syncSteadfastStatusForOrder.rejected, (state, action) => {
        state.actionLoading = false;
        state.error = action?.payload?.message || "Failed to sync Steadfast status";
      });
  },
});

export const { resetOrderDetails, clearAdminOrderMessages } = adminOrderSlice.actions;
