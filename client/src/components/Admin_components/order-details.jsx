...
//             buttonText={actionLoading ? "Updating..." : "Update Order Price"}
...
import {
  ...
  updateOrderStatus,
  updatePaymentStatus,
  updateOrderPrice, // ✅ ADD THIS ACTION IN YOUR SLICE
  createSteadfastParcelForOrder,
  syncSteadfastStatusForOrder,
} from "@/store/admin/order-slice";
import { toast } from "sonner";
...
function AdminOrderDetailsView({ orderDetails }) {
  ...
  }, [orderDetails?._id, orderDetails?.totalAmount]);

  if (!orderDetails) return null;

  // ✅ Order Price update (NEW)
  const handleUpdateOrderPrice = (event) => {
    event.preventDefault();

    const raw = formData.orderPrice;

    if (raw === "" || raw === null || raw === undefined) {
      toast.error("Order price is required");
      return;
    }
    ...
    if (price < 0) {
      toast.error("Order price cannot be negative");
      return;
    }
  ...
  const phoneForTel = String(phoneRaw).replace(/[^\d+]/g, "");
    ...
    <DialogContent className="sm:max-w-[700px] max-h-[85vh] overflow-y-auto">
      <div className="grid gap-6">
        ...
        <div className="grid gap-2">
          ...
          {/* ✅ This will update after DB update + refetch */}
          <div className="flex mt-2 items-center justify-between">
            <p className="font-medium">Order Price</p>
            <Label>৳{orderDetails?.totalAmount}</Label>
          </div>
        ...
        <div className="grid gap-2">
          ...
          <div className="grid gap-0.5 text-black">
            ...
            {/* ✅ Phone icon + clickable number (opens dialer) */}
            {phoneRaw ? (
              <a
        ...
        {/* ✅ Order Price update (numeric + autofill + updates DB + refresh header) */}
        <div>
          <CommonForm
            formControls={[
              {
            ...
            setFormData={setFormData}
            buttonText={actionLoading ? "Updating..." : "Update Order Price"}
            onSubmit={handleUpdateOrderPrice}
          />
        </div>
